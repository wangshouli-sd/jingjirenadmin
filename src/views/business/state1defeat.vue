          ...
          <el-row>
            ...
            <el-col :span="18" class="ywitem-r">
              <div class="">
                ...
                <div>
                  <el-tag v-for="item in scope.row.tags" :key="item" type="success" size="mini" style="margin-right: 5px">{{ item }}</el-tag>
                </div>
              </div>
            </el-col>
          </el-row>
...
import { state1defeatlist, state1defeatdel } from '@/api/business/state1defeat.js' // 获取列表、删除
...
export default {
  ...
  methods: {
    getList() {
      state1defeatlist(this.listQuery).then((res) => {
        this.list = res.list
        this.page = res.page
      })
    },
    ...
    handleDelete(row) {
      this.$confirm('此操作将删除该项目相关信息, 是否继续?', '提示', {
        ...
        state1defeatdel(row.id).then((res) => {
    ...
    handleBatchDelete() {
      ...
      this.$confirm('此操作将删除所选项目相关信息, 是否继续?', '提示', {
        ...
        state1defeatdel(mulid.join(',')).then((res) => {
          if (res.status.state === 'success') {
            this.getList() // 删除成功，重新加载列表
            this.$message({
              type: 'success',
