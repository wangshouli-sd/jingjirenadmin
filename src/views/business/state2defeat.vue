  ...
  <div class="app-container" style="">
    ...
    <el-table
      ...
      <el-table-column prop="state" label="状态" width="120" align="center">
        <template slot-scope="scope">
          <el-tag type="danger">服务终止</el-tag>
        </template>
      </el-table-column>
...
import { state2defeatlist, state2defeatdel } from '@/api/business/state2defeat.js' // 获取列表、删除
...
export default {
  ...
  methods: {
    getList() {
      state2defeatlist(this.listQuery).then((res) => {
        this.list = res.list
        this.page = res.page
      })
    },
    ...
    handleDelete(row) {
      this.$confirm('此操作将删除该项目相关信息, 是否继续?', '提示', {
        ...
        state2defeatdel(row.id).then((res) => {
    ...
    handleBatchDelete() {
      ...
      this.$confirm('此操作将删除所选项目相关信息, 是否继续?', '提示', {
        ...
        state2defeatdel(mulid.join(',')).then((res) => {
          if (res.status.state === 'success') {
            this.getList()
            this.$message({
              type: 'success',
